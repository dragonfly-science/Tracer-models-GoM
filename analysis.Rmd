...
bind_rows(cc_list, .id = 'Predator') %>%
  ggplot() + 
  geom_point(aes(x=FA, y=avg_CC, col=Predator)) + 
  geom_linerange(aes(x=FA, y=avg_CC, ymax=qgamma(0.95,shape = avg_CC^2/SD_CC^2, rate = avg_CC/SD_CC^2), ymin=qgamma(0.05,shape = avg_CC^2/SD_CC^2, rate = avg_CC/SD_CC^2), col=Predator), linetype=3) + 
  cowplot::theme_cowplot() + 
  scale_y_log10() + 
  geom_hline(yintercept = 1, linetype=3) + 
  coord_cartesian(ylim=c(0.1,NA)) + 
...
KF.data.PS <- add_FA(FA.predators = as.matrix(KF_PS_pred[, FA_cols]),
                     ...
                     CC.mean = cc_list$`Killifish`$avg_CC,
                     CC.var  = cc_list$`Killifish`$SD_CC)
